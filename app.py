# avails_bot.py — Safe boot, LLM optional, robust loaders, sums-after-filters, intents
...
# ========= Streamlit page =========
st.set_page_config(page_title="Avails Bot (Beta)", layout="wide")
st.title("Avails Bot (Beta Testing version)")

# ========= Config =========
USE_LLM = True  # will auto-disable if key or SDK missing

# ========= Safe OpenAI init (never crash) =========
_client = None
_openai_error = None
try:
    ...
    if not _api_key:
        try:
            _api_key = st.secrets.get("OPENAI_API_KEY", None)  # no KeyError
        except Exception as e:
            _openai_error = f"Secrets access failed: {e}"
    if _api_key:
        try:
            from openai import OpenAI  # requires openai>=1.40
            _client = OpenAI(api_key=_api_key)
        except Exception as e:
            _openai_error = f"OpenAI SDK import/init failed: {e}"
    else:
        _openai_error = _openai_error or "OPENAI_API_KEY not found (env or secrets)."
except Exception as e:
    _openai_error = f"OpenAI init error: {e}"

if _client is None:
    USE_LLM = False

st.caption(f"🔧 Parser mode: **{'LLM mode' if USE_LLM else 'Heuristic mode'}**")
if not USE_LLM and _openai_error:
    st.sidebar.info(f"LLM disabled: {_openai_error}")
...
def load_df(region: str) -> pd.DataFrame:
    wanted = []
    if region in ("NA","GLOBAL"):   wanted.append(("na_avails.xlsx","NA"))
    if region in ("APAC","GLOBAL"): wanted.append(("apac_avails.xlsx","APAC"))

    frames, errors = [], []
    for path, label in wanted:
        try:
            if not os.path.exists(path):
                errors.append(f"{label}: '{path}' not found in app working dir.")
                continue
            # You can trim columns with usecols=[...] to speed boot
            ...
            errors.append(f"{label}: failed to load '{path}': {e}")

    if errors:
        with st.expander("Dataset load warnings"):
            for e in errors:
                st.error(e)
...
def map_categories_with_llm(raw_terms: List[str], vocab: List[str]) -> List[str]:
    if not raw_terms or not USE_LLM or _client is None:
        return []
    try:
...
BAD_JOUNCE = {"cheap reach", "made for advertising", "made for ads", "made for advertizing", "mfa", "untrusted", "untrsuted"}

def apply_macros(df: pd.DataFrame, macros: List[str]) -> pd.DataFrame:
    d = df
    macros = macros or []
    if "premium" in macros and "Jounce Media" in d.columns:
...
if user_input:
    ...
    if missing or df.empty:
        st.error("No data loaded or missing required headers. Ensure files exist in repo root: "
                 "`na_avails.xlsx`, `apac_avails.xlsx` (case-sensitive). Missing: " + ", ".join(missing))
        st.stop()
    ...
    # Parse
    def VIDEO_SUPPLY_ACTIVE(prompt: str) -> bool:
        t = (prompt or "").lower()
        return bool(VIDEO_SUPPLY_PAT.search(t)) and ("rewarded" not in t)

    parse_info = extract_query_with_errors(user_input)
    q = parse_info.get("parsed") or {}

    # Hard-enforce vertical from text
    t = (user_input or "")
    ...
    # Video supply override
    video_supply_mode = VIDEO_SUPPLY_ACTIVE(user_input)
    ...
    # Vertical truth first
    d = compute_vertical_truth(d)
    ...
    # Determine requested includes in DF space
    ...
    if intent == "count":
        if video_supply_mode:
            st.info("Note: Some FSI request slots are video-ready; normalization applied in the view.")
        answer_count(g)
        st.download_button("Download CSV", g.to_csv(index=False).encode("utf-8"), "avails_app_level.csv", "text/csv")
        st.stop()
    if intent == "breakdown":
